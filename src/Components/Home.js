...
export default class Home extends Component {
    ...
    handleLocation = (e) => {
        ...
        // this.setState({ location: e.target[0].value })
    }
    handleChange = ({ target }) => {
        if (target.value.length > 3) {
            this.setState({ location: target.value, button: false })

        }
    }
    handleLogin = (e) => {
        e.preventDefault()
        console.log(this.props.loginData.SwiggyReducer.signupData)
        const loginCheck = this.props.loginData.SwiggyReducer.signupData.find(obj => obj.phone === e.target[0].value && obj.password === e.target[1].value)
        if (loginCheck) {
            // alert('hi')
            console.log(loginCheck.name)
            this.setState({ user: loginCheck.name, loginCheck: true, onLogin: true })
        }
    ...
    handleSignUp = (e) => {
        e.preventDefault()
        const check = this.props.loginData.SwiggyReducer.signupData.filter(obj => obj.phone === e.target[0].value)
        console.log(check)

        if (check.length > 0) {
            this.setState({ userCheck: true, signup: false })
            e.target[0].value = ''
            e.target[2].value = ''
            e.target[3].value = ''
        } else {
            ...
            this.props.categoryHandler(signUpdata)
            this.setState({ signup: true, userCheck: false })
            e.target[0].value = ''
            e.target[1].value = ''
            e.target[2].value = ''
            e.target[3].value = ''
        ...
        if (this.state.wait) {
            ...
            return (
                <>
                    ...
                    {this.state.loginCheck && <div className="container p-2">
                        <div className="row">
                            ...
                            <div className="col align-self-center">
                                <ul className="list-inline d-flex gap-5" >
                                    ...
                                    <li>
                                        ...
                                        <span className="ms-2">{this.state.user}</span>
                                    </li>
                                    <li>
                    ...
                    <div className="d-flex justify-content-end">
                        ...
                        <div className="d-flex">
                            <div className="mt-5 pt-5 pe-5 w-50">
                                <div className="d-flex justify-content-between">
                                    ...
                                    <div>
                                        <div className="d-flex gap-5">
                                            <div>
                                                {/* login */}
                                                <button className="btn " type="button" data-bs-toggle="offcanvas" data-bs-target="#offcanvasRight" aria-controls="offcanvasRight" hidden={this.state.onLogin}>Login</button>

                                                <div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasRight" aria-labelledby="offcanvasRightLabel">
                                                    <div class="offcanvas-header">
                                                        <h5 id="offcanvasRightLabelLogin">Login</h5>

                                                        <button type="button" class="btn-close text-reset" data-bs-dismiss="offcanvas" aria-label="Close"></button>
                                                    </div>
                                                    ___
                                                    <div class="offcanvas-body">
                                                        <form className="d-flex flex-column gap-1" onSubmit={this.handleLogin}>
                                                            <input type="phone" placeholder="Phone number" className="pt-3 pb-3"></input>
                                                            <input type="password" placeholder="Password" className="pt-3 pb-3"></input>
                                                            <button style={{ background: "#FC8019" }} className="btn" >login</button>

                                                        </form>
                                                        <span style={{ fontSize: "12px" }} className="text-muted">By clicking on Login, I accept the Terms & Conditions & Privacy Policy</span>
                                                    </div>
                                                </div>
                                            </div>
                                            {/* singUp button */}
                                            <div >
                                                <button class="btn btn btn-dark pe-3 ps-3" type="button" data-bs-toggle="offcanvas" data-bs-target="#offcanvasRighttwo" aria-controls="offcanvasRight" hidden={this.state.onLogin}>Sign up</button>

                                                <div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasRighttwo" aria-labelledby="offcanvasRightLabel">
                                                    <div class="offcanvas-header">
                                                        <h5 id="offcanvasRightLabelSignUp">Sign up</h5>
                                                        <button type="button" class="btn-close text-reset " data-bs-dismiss="offcanvas" aria-label="Close"></button>
                                                    </div>
                                                    ____
                                                    <div class="offcanvas-body">
                                                        <form className="d-flex flex-column" onSubmit={this.handleSignUp}>
                                                            <input type="phone" placeholder="Phone number" className="pt-3 pb-3 border" required></input>
                                                            <input type="text" placeholder="Name" className="pt-3 pb-3 border" required></input>
                                                            <input type="email" placeholder="Email" className="pt-3 pb-3 border" required></input>
                                                            <input type="password" placeholder="Password" className="pt-3 pb-3 border" required></input>
                                                            <input type="text" placeholder="Referral code" className="pt-3 pb-3 border" required></input>

                                                            <button style={{ background: "#FC8019" }} className="btn mt-3" >Continue</button>

                                                        </form>
                                                        <span style={{ fontSize: "12px" }} className="text-muted">By creating an account, I accept the  I accept the Terms & Conditions & Privacy Policy</span>
                                                        {this.state.signup && <div>Signedup successfully. Login Now</div>}
                                                        {this.state.userCheck && <div>User Already exists</div>}
                                ...
                                <div>
                                    <div className="row align-self-start row-cols-1 mt-5">
                                ...
                                <div className="mt-4">
                                    ...
                                    <input onChange={this.handleChange} className="p-3" type="text" placeholder="Enter your delivery location" list="location"></input>
                                    ...
                                    <Link to={`/restaurants/${this.state.location}`}><button style={{ background: "#FC8019" }} className="btn p-3 m-0" disabled={this.state.button}>FIND FOOD</button></Link>
                                    {/* </form> */}
                                </div>
                                <div>
                                    {/* popular cities */}
                                    <div className="col text-left text-wrap mb-1 text-uppercase h6 text-muted mt-4">
                    ...
                    <>
                        <div style={{ background: '#2b1e16', padding: '30px 20px' }}>
                    ...
                    {/* ..............third data */}
                    <div className="d-flex justify-content-center">
                        <div className="mt-5 pt-5 ms-5 me-5 pe-5">
                            <div >
